...
class AuthManager {
    ...
    // ====================
// SOCIAL LOGIN (Mocked Google / Facebook)
// ====================
setupSocialLogin() {
    const googleBtn = document.querySelector(".google-btn");
    const facebookBtn = document.querySelector(".facebook-btn");

    const removePopup = () => document.querySelector(".fake-popup")?.remove();
    ...
    const openGooglePopup = () => {
        ...
        popup.innerHTML = `
            <div class="popup-content">
                ...
                <div class="account-option" data-user="dummy@goalforge.com">
                    <img src="images/Profile.png" alt="">
                    <span>dummy@goalforge.com</span>
                </div>
                <div class="account-option" data-user="guest@goalforge.com">
                    <img src="images/Profile.png" alt="">
                    <span>guest@goalforge.com</span>
                </div>
        ...
        popup.querySelectorAll(".account-option").forEach(acc => {
            acc.addEventListener("click", () => {
                const fakeUser = {
                    id: this.generateId(),
                    email: acc.dataset.user,
                    displayName: acc.dataset.user.split("@")[0],
                    createdAt: new Date().toISOString(),
                    lastLogin: new Date().toISOString()
                };
                this.loginUser(fakeUser);
                this.showMessage(Signed in as ${fakeUser.displayName}, "success");
                removePopup();
                setTimeout(() => window.location.href = "dashboard.html", 1000);
            });
        });

        popup.querySelector(".cancel-popup").addEventListener("click", removePopup);
    };
    ...
    const openFacebookPopup = () => {
        removePopup();
        const popup = document.createElement("div");
        popup.className = "fake-popup fb-popup";
        popup.innerHTML = `
            <div class="fb-content">
                ...
                <h3>Facebook will receive:</h3>
                <p class="fb-receive">your public profile and email address.</p>
                <button class="fb-continue">Continue as User</button>
        ...
        popup.querySelector(".fb-continue").addEventListener("click", () => {
            const fakeUser = {
                id: this.generateId(),
                email: "user_facebook@example.com",
                displayName: "User",
                createdAt: new Date().toISOString(),
                lastLogin: new Date().toISOString()
            };
            this.loginUser(fakeUser);
            this.showMessage(Logged in as ${fakeUser.displayName}, "success");
            removePopup();
            setTimeout(() => window.location.href = "dashboard.html", 1000);
        });

        popup.querySelector(".fb-cancel").addEventListener("click", removePopup);
    };
